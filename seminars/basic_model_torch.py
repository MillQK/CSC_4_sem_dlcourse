...
class BasicTranslationModel(nn.Module):
    ...
    def encode(self, inp, **flags):
        ...
        end_index = infer_length(inp, self.inp_voc.eos_ix, type=torch.cuda.LongTensor)
        end_index[end_index >= inp.shape[1]] = inp.shape[1] - 1
        enc_last = enc_seq[torch.cuda.LongTensor(range(enc_seq.shape[0])), end_index.detach(), :]
...
def to_one_hot(y, n_dims=None):
    ...
    return Variable(y_one_hot).cuda() if isinstance(y, Variable) else y_one_hot
